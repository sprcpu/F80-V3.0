...
local function create_group(msg)
    ...
    if is_sudo(msg) or is_realm(msg) and is_admin1(msg) then
		...
		return 'Group :\n[#'..string.gsub(group_name, '_', ' ')..'] has been created.'
	end
end
...
local function admin_promote(msg, admin_id)
	...
	if data[tostring(admins)][tostring(admin_id)] then
		return admin_id..' Was #Gadmin'
	end
	data[tostring(admins)][tostring(admin_id)] = admin_id
	save_data(_config.moderation.data, data)
	return admin_id..' is Now #Gadmin'
end
...
local function admin_demote(msg, admin_id)
	...
	if not data[tostring(admins)][tostring(admin_id)] then
		return admin_id..' Was Not #Gadmin'
	end
	data[tostring(admins)][tostring(admin_id)] = nil
	save_data(_config.moderation.data, data)
	return admin_id..' Now is Not #Gadmin'
end
...
local function admin_list(msg)
	...
	local message = '#Gadmin OF @F80_TG:\n'
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
            return send_large_msg(receiver, '@'..member_username..' was #Gadmin.')
        end
        data['admins'][tostring(member_id)] = member_username
        save_data(_config.moderation.data, data)
	return send_large_msg(receiver, '@'..member_username..' is now #Gadmin.')
end
...
function run(msg, matches)
    ...
    if matches[1]:lower() == 'mkgp' and matches[2] then
    ...
    local receiver = get_receiver(msg)
			...
			if matches[1]:lower() == 'setabout' and matches[2]:lower() == 'sgroup'and is_realm(msg) then
		...
		if matches[1]:lower() == 'kill' and matches[2] == 'chat' and matches[3] then
			...
			if is_realm(msg) then
				...
				return '#Error_404: Group '..matches[3]..' not found'
			end
		end
		if matches[1]:lower() == 'kill' and matches[2] == 'realm' and matches[3] then
			...
			if is_realm(msg) then
				...
				return '#Error_404: Realm '..matches[3]..' not found'
			end
		end
