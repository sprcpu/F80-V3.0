...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return '#Tgservice was Clean'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return '#Tgservice Now Cleaning By bot'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return '#TgService was Not Clean'
  else
...
local function enable_chat(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_chat = data[tostring(target)]['settings']['chat']
...
local function disable_chat(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_strict_lock = data[tostring(target)]['settings']['chat']
  if group_chat == 'no' then
    return 'bot #chat >OFF'
  else
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return '#rules set  BY [#'..string.gsub(msg.from.print_name, "_", " ")..']{'..(msg.from.username or ' '..msg.from.id..' ')..'}'
end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return '|<code>SPR-CPU</code>|\nDev : @SILENT_POKER \n Sudo : @pharmakill\nChannel : @SPRCPU_Team\nUse <code>Setrules</code> to Set rules!'
  end
...
local function modlist(msg)
  ...
  for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
    message = message ..i..'>'..v..' <code>[' ..k.. ']</code> \n'
    i = i + 1
  end
  return message
end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1]:lower() == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '🌚 Create a link using  <code>newlink</code>  first! \n\n🌝 Or if I am not creator use  ‼setlink‼  to set your link ')
		...
		if matches[1]:lower() == 'link' then
			...
			if not group_link then
				return "🌚 Create a link using <code>newlink</code> first! \n\n🌝 Or if I am not creator use ‼setlink‼ to set your link "
			end
		...
		--[[if matches[1] == 'kick' and is_momod(msg) then
			...
			elseif matches[1] == '+admin' and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'admin' and not string.match(matches[2], '^%d+$') then
				--[[local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'setadmin'
				}
		...
		if matches[1]:lower() == '+mod' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == '+mod' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'modprom'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] modpromd user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == '+mod' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'modprom',
				}
		...
		if matches[1]:lower() == '-mod' then
			...
			elseif matches[1]:lower() == '-mod' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1]:lower() == 'setphoto' and is_momod(msg) then
			...
			return '|<code>SEND PHOTO NOW</code>|'
		end
	  ...
	  if matches[2] == 'english' then
            ...
            if matches[2] == 'botchat' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked enabled chat settings")
				return enable_chat(msg, data, target)
			end
		end
	  ...
	  if matches[2] == 'english' then
            ...
            if matches[2] == 'botchat' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked disabled chat settings")
				return disable_chat(msg, data, target)
			end
		end
